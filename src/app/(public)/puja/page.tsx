...
export default async function PujaPage() {
  ...
  const eventName  = featured?.name ?? festivalConfig.name
  const eventYear  = featured?.year ?? festivalConfig.year
  const apiDays    = featured?.days ?? []

  // Use API days when available; fall back to festivalConfig.days.
  const useFallback = apiDays.length === 0
  const days = useFallback
    ? festivalConfig.days.map((d) => ({
        key:         d.key,
        label:       d.label,
        emoji:       d.emoji,
        date:        d.date,
    ...
    <>
      ...
      <section className="py-20 bg-white">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          <SectionHeading
            label="Schedule"
            title={`Puja ${eventYear}`}
            className="mb-12"
          />

          <div className="flex flex-col gap-8">
            {days.map(({ key, label, emoji, date, rituals, highlight }, idx) => (
