                ...
                return (
                  <div key={key} className="group relative flex gap-6">
                    ...
                    <div className="pb-8 flex-1">
                      ...
                      {items.length > 0 && (
                        hasTimes ? (
                          <div className="mt-2 rounded-xl border border-brand-orange/10 bg-brand-orange/5 divide-y divide-brand-orange/10 overflow-hidden">
                            {items.map((act, i) => (
                              <div key={i} className="flex items-start gap-4 px-4 py-2.5">
                                <span className="font-mono text-xs font-semibold text-brand-orange w-16 shrink-0 pt-0.5">
                                  {act.time ? fmtTime(act.time) : '—'}
                                </span>
                                <div className="flex flex-col">
                                  <span className="text-sm text-brand-navy">{act.name}</span>
                                  {act.description && (
                                    <span className="text-[11px] text-muted-foreground italic">{act.description}</span>
                                  )}
                                </div>
                              </div>
                            ))}
                          ...
                          <div className="flex flex-wrap gap-2 mt-2">
                            {items.map((act, i) => (
                              <span key={i} className="text-xs px-3 py-1 rounded-full bg-brand-orange/8 text-brand-navy border border-brand-orange/15 font-medium">
                                {act.name}
                              </span>
                            ))}
                          </div>
            ...
            <div className="columns-2 sm:columns-3 lg:columns-4 gap-3">
              {event.gallery.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="mb-3 relative w-full rounded-xl overflow-hidden break-inside-avoid bg-brand-orange/5"
                  >
                    {imgSrc ? (
                      <Image
                        src={imgSrc}
